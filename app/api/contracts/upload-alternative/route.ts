...
export async function POST(request: NextRequest) {
  try {
    ...
    // Tentar upload usando o cliente com bypass de RLS (apenas para admins)
    let uploadData, uploadError
    
    try {
      // Primeiro, tentar upload normal
    ...
    if (uploadError) {
      ...
      if (uploadError.message?.includes('row-level security') || uploadError.message?.includes('policy')) {
        return NextResponse.json(
          { 
            success: false, 
            error: "Erro de permissão no storage. Verifique se as políticas de storage foram configuradas corretamente.",
